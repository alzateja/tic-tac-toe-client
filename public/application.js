...
/***/ function(module, exports, __webpack_require__) {
	...
	var userAuthEvents = __webpack_require__(12);
	// On document ready
...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameLogic = __webpack_require__(8);
	...
	var over = false;
	var winCondition = '';
	var xOptimalMove = null;
	var oOptimalMove = null;
	...
	var newGame = function newGame(data) {
	  ...
	  gameApi.createGames().then(ui.createGameSuccess).catch(ui.createGameFailure);
	  $('#game-board').show();
	  ui.boardClear();
	  ui.onNewGame();
	  board = ['', '', '', '', '', '', '', '', ''];
	...
	var capturePlayerAction = function capturePlayerAction(event) {
	  event.preventDefault();
	  // Store selection to pass as argument
	  var elementId = $(this).attr('id');
	  var num = +elementId.replace('board', '');
	...
	var onPlayerAction = function onPlayerAction(num) {
	  if (gameTied || gameWon) {
	...
	// Step 4 - Update the board and rerender. Increment Move Counter
	var boardUpdate = function boardUpdate(selection) {
	...
	//  Step 6  -- FORMAT DATA TO SEND TO BACK END SERVER
	var updateGameInfoServer = function updateGameInfoServer(num) {
	  ...
	  gameApi.gameUpdated(dataForUpdate, gameId).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
	};

	// STEP 7 - PROCESS THE OUTCOME OF PRECEDING STEPS AND NOTIFY
	var gameOutcome = function gameOutcome(player) {
	...
	// STEP 8 SWITCH PLAYERS AND UPDATE COUNTER
	var playerMoveCount = function playerMoveCount() {
	...
	var turnAiOff = function turnAiOff() {
	  console.log(2);
	  ai = 'off';
	  $('#ai-menu').text('AI Off');
	};

	var turnOnDumbAi = function turnOnDumbAi() {
	  console.log(1);
	  ai = 'dumb';
	...
	var turnOnSmartAi = function turnOnSmartAi() {
	  console.log(3);
	  ai = 'smart';
	...
	var turnOnSmartestAi = function turnOnSmartestAi() {
	  console.log(3);
	  ai = 'smartest';
...
/***/ function(module, exports) {
	...
	var openMoves = [];
	var winCondition = '';
	var xWinnableOneMove = [];
	var oWinnableOneMove = [];
	var xWinnableTwoMove = [];
	var oWinnableTwoMove = [];
	var xMoveValues = {};
	var oMoveValues = {};
	var xOptimalMove = null;
	var oOptimalMove = null;
	...
	var gameWon = false;
	var gameTied = false;
	var profileTotalGames = 0;
	var profileWins = 0;
	var profileTies = 0;
	var profileLosses = 0;
	var profileUnfinishedGames = 0;
	...
	var calculateGameStats = function calculateGameStats(data) {
	  ...
	  console.log('Total: ' + profileTotalGames + ' , Wins: ' + profileWins + ' , Losses: ' + profileLosses + ', Ties: ' + profileTies + ', DNF: ' + profileUnfinishedGames);
	...
	var runLogic = function runLogic(board) {
	  // RESET VARIABLES
	  openMoves = [];
	  winCondition = '';
	  xWinnableOneMove = [];
	...
	//  1 -  Identify Possible outstanding moves
	var determineOpenMoves = function determineOpenMoves(board) {
	...
	var evaluateBoard = function evaluateBoard(board) {
	  var unwinnableCount = 0;
	  var x = [];
	  var o = [];
	  var blank = [];
	  for (var prop in allWins) {
	    x = [];
	    o = [];
	    blank = [];
	...
	// 3 - Calcuate the Value of Moves and put in Object
	var CalculateMoveValues = function CalculateMoveValues() {
	  xMoveValues = {};
	  oMoveValues = {};
	  // BEGIN X CALC
	  // X - 2 move count and value = VALUE of 1 for each move
	  if (xWinnableTwoMove.length > 0) {
	    for (var i = 0; i < xWinnableTwoMove.length; i++) {
	  ...
	  if (xWinnableOneMove.length > 0) {
	    for (var _i = 0; _i < xWinnableOneMove.length; _i++) {
	      if (xMoveValues[xWinnableOneMove[_i]] === undefined) {
	        xMoveValues[xWinnableOneMove[_i]] = 0;
	      }
	      if (oMoveValues[xWinnableOneMove[_i]] === undefined) {
	        oMoveValues[xWinnableOneMove[_i]] = 0;
	      }
	      // console.log(xMoveValues)
	      // console.log(oMoveValues)
	      xMoveValues[xWinnableOneMove[_i]] += 9;
	      oMoveValues[xWinnableOneMove[_i]] += 6;
	      // console.log(xMoveValues)
	      // console.log(oMoveValues)
	    }
	  }
	  ...
	  if (oWinnableTwoMove.length > 0) {
	    for (var _i2 = 0; _i2 < oWinnableTwoMove.length; _i2++) {
	      if (oMoveValues[oWinnableTwoMove[_i2]] === undefined) {
	        oMoveValues[oWinnableTwoMove[_i2]] = 0;
	      }
	      oMoveValues[oWinnableTwoMove[_i2]]++;
	  ...
	  if (oWinnableOneMove.length > 0) {
	    for (var _i3 = 0; _i3 < oWinnableOneMove.length; _i3++) {
	      if (oMoveValues[oWinnableOneMove[_i3]] === undefined) {
	        oMoveValues[oWinnableOneMove[_i3]] = 0;
	      }
	      if (xMoveValues[oWinnableOneMove[_i3]] === undefined) {
	        xMoveValues[oWinnableOneMove[_i3]] = 0;
	      }
	      // console.log(xMoveValues)
	      // console.log(oMoveValues)
	      oMoveValues[oWinnableOneMove[_i3]] += 9;
	      xMoveValues[oWinnableOneMove[_i3]] += 6;
	      // console.log(xMoveValues)
	      // console.log(oMoveValues)
	    }
	  }
	...
	// 4- Calculate the Optimal Move based on
	var pickOptimalMove = function pickOptimalMove() {
	  var xhighestValuesKeys = [];
	  var ohighestValuesKeys = [];
	  var sharedHighestValuesKeys = [];
	  var xhighestValuesNumber = 0;
	  var ohighestValuesNumber = 0;
	  // let counter = 0
	  var num = null;

	  // DEDUPE AND DETERMINE HIGHEST MOVES - PLAYER O
	  for (var prop in oMoveValues) {
	    // counter++
	  ...
	  for (var _prop in xMoveValues) {
	    // counter++
	    // console.log(counter + ': move - ' + prop + ' , value - ' + xMoveValues[prop])
	    // reset if higher value found
	    if (xMoveValues[_prop] > xhighestValuesNumber) {
	      // console.log(xhighestValuesKeys)
	      xhighestValuesKeys = [];
	      xhighestValuesNumber = xMoveValues[_prop];
	    }
	    // post higher value moves
	    if (xMoveValues[_prop] === xhighestValuesNumber) {
	      // console.log(xhighestValuesKeys)
	      xhighestValuesKeys.push(_prop);
	    }
	  }
	  //  END OF PARING DOWN DESCISIONS
	  var tempStore = null;
	  if (ohighestValuesKeys.length !== 0 && xhighestValuesKeys.length !== 0) {
	    for (var o = 0; o < ohighestValuesKeys.length; o++) {
	      tempStore = ohighestValuesKeys[o];
	      for (var x = 0; x < xhighestValuesKeys.length; x++) {
	...
	var returnXOptimalMove = function returnXOptimalMove() {
	  return xOptimalMove;
	};
	var returnOOptimalMove = function returnOOptimalMove() {
	  return oOptimalMove;
	};
	var returnOptimalMove = function returnOptimalMove() {
	  return optimalMove;
	};
	var returnGameTied = function returnGameTied() {
	  return gameTied;
	};
	var returnGameWon = function returnGameWon() {
	  return gameWon;
	};
	var returnWinCondition = function returnWinCondition() {
	  return winCondition;
	};

	var returnTotalWins = function returnTotalWins() {
	  return profileWins;
	};
	var returnTotalLosses = function returnTotalLosses() {
	  return profileLosses;
	};
	var returnTotalTies = function returnTotalTies() {
	  return profileTies;
	};
	var returnTotalGames = function returnTotalGames() {
	  return profileTotalGames;
	};

	var returnTotalUnfinished = function returnTotalUnfinished() {
	  return profileUnfinishedGames;
	};

	module.exports = {
	  runLogic: runLogic,
	  returnWinCondition: returnWinCondition,
	  returnGameWon: returnGameWon,
	  returnGameTied: returnGameTied,
	  returnXOptimalMove: returnXOptimalMove,
	  returnOOptimalMove: returnOOptimalMove,
	  returnOptimalMove: returnOptimalMove,
	  calculateGameStats: calculateGameStats,
	  returnTotalGames: returnTotalGames,
	  returnTotalTies: returnTotalTies,
	  returnTotalLosses: returnTotalLosses,
	  returnTotalWins: returnTotalWins,
	  returnTotalUnfinished: returnTotalUnfinished
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var boardClear = function boardClear() {
	  $('#board0').html('');
	  $('#board1').html('');
	  $('#board2').html('');
	  $('#board3').html('');
	...
	var boardRender = function boardRender(board) {
	  for (var i = 0; i < board.length; i++) {
	    if (board[i] === 'o') {
	...
	// NOTIFY USER
	var notifyTurn = function notifyTurn(player) {
	  // console.log('Whose turn')
	  $('#game-messaging').text('Player ' + player.toUpperCase() + ' your turn');
	};
	// NOTIFY USER
	var notifyNewGameNeeded = function notifyNewGameNeeded(player) {
	  // console.log('Whose turn')
	  $('#game-messaging').text('Click New Game');
	};

	// NOTIFY USER OF TIE
	var notifyTie = function notifyTie() {
	  $('#game-messaging').text('Tied game');
	};

	var notifyWin = function notifyWin(player) {
	...
	var infoSuccess = function infoSuccess(data) {
	  console.log('get game ', data);
	  store.games = data.games;
	};

	var infoFailure = function infoFailure(error) {
	  console.error('get game', error);
	};

	var updatePlayerRecord = function updatePlayerRecord() {
	  console.log('Executing update');
	  gameApi.getFinishedGames().then(infoSuccess).catch(updateGameFailure);
	};
	...
	module.exports = {
	  ...
	  notifyNewGameNeeded: notifyNewGameNeeded
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameJoined = function gameJoined(data) {
	  return $.ajax({
	...
	var gameUpdated = function gameUpdated(data, id) {
	  return $.ajax({
	...
	var findGame = function findGame() {
	  return $.ajax({
	    url: config.apiOrigin + '/games/' + store.game.id,
	    method: 'GET',
	    headers: {
